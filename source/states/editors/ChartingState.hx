...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	{
		var autoSavetimer:FlxTimer = new FlxTimer();

		autoSavetimer.start(180, autoSaveOnTimer);
	...
	function autoSaveOnTimer(timer:FlxTimer):Void
	{
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				LoadingState.loadAndSwitchState(new states.editors.EditorPlayState(sectionStartTime(), _song.mania));
			}
	...
	private function loadJsonPopup()
	{
		_load = new FileReference();
		_load.addEventListener(Event.SELECT, selectJsonFile);

		var Filter = new FileFilter("JSON Files", "*.json");
		_load.browse([Filter]);
	}

	function selectJsonFile(_):Void
	{
		_load.addEventListener(Event.COMPLETE, onLoadJsonComplete);
		_load.addEventListener(IOErrorEvent.IO_ERROR, onLoadJsonError);
		_load.load();
	}

	function onLoadJsonError(_):Void
	{
		_load.removeEventListener(Event.COMPLETE, onLoadJsonComplete);
		_load.removeEventListener(IOErrorEvent.IO_ERROR, onLoadJsonError);
		_load = null;
		FlxG.log.error("Problem loading Level data");
	}

	function onLoadJsonComplete(_):Void
	{
		trace("Test");
		_load.removeEventListener(Event.COMPLETE, onLoadJsonComplete);
		_load.removeEventListener(IOErrorEvent.IO_ERROR, onLoadJsonError);
